...
!function($){"use strict";$us.ToggleMoreContent=function(container){this.init(container)};$us.ToggleMoreContent.prototype={init:function(container){this.$container=$(container);this.$firstElm=$('> *:first',this.$container);this.toggleHeight=parseInt(this.$container.data('toggle-height')||200);this.initHeightCheck();this.$container.on('click','.toggle-show-more, .toggle-show-less',function(e){e.preventDefault();e.stopPropagation();this.$container.toggleClass('show_content',$(e.target).hasClass('toggle-show-more'));setTimeout(function(){$us.$canvas.trigger('contentChange')},1)}.bind(this))},initHeightCheck:function(){if(this.$firstElm.height()<=this.toggleHeight){$('.toggle-links',this.$container).hide();this.$firstElm.css('height','')}else{$('.toggle-links',this.$container).show();this.$firstElm.css('height',this.toggleHeight)}}};$.fn.usToggleMoreContent=function(){return this.each(function(){$(this).data('usToggleMoreContent',new $us.ToggleMoreContent(this))})};$('.with_show_more_toggle').usToggleMoreContent()}(jQuery);!function($){"use strict";function USScroll(options){var defaults={attachOnInit:'\
				.menu-item a[href*="#"],\
				.menu-item[href*="#"],\
...
$.each(this.breakpoints,function(breakpointWidth,breakpointArgs){if(breakpointArgs!==undefined&&breakpointArgs.items!==undefined){this.carouselOptions.responsive[breakpointWidth]=breakpointArgs;this.carouselOptions.responsive[breakpointWidth].items=parseInt(breakpointArgs.items)}}.bind(this));this.$list.owlCarousel(this.carouselOptions);this.$container.find('.with_show_more_toggle').each(function(){$(this).data('usToggleMoreContent').initHeightCheck()})}.bind(this))}
